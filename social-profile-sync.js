/* ============================================================
   طرف الخيط — Social Profile Sync (loop-safe)
   ...
   - بدون MutationObserver على صفحة البروفايل.
   ============================================================ */
(function(){
  'use strict';
  ...
  async function rest(path, session, body, prefer){
    ...
    const res=await fetch(SUPABASE_URL+'/rest/v1/'+path,{
      method:'POST',headers,body:JSON.stringify(body==null?{}:body)
    });
    if(!res.ok){
  ...
  async function syncProfile(session){
    ...
    try{await rest('game_profiles?on_conflict=user_id',session,[payload],'resolution=merge-duplicates,return=minimal');}
    catch(err){console.warn('social profile sync:',err&&err.message||err);}
  ...
  async function syncFacebookFriends(session){
    ...
    }catch(err){console.warn('facebook friends sync skipped:',err&&err.message||err);}
  }
  ...
  function paintProfileAvatar(session){
    ...
    img.src=url;img.alt='صورة المحقق';img.referrerPolicy='no-referrer';
  ...
  function boot(){
    ...
    // انتظار محدود لإعادة رسم بطاقة الهوية، بدون مراقبة DOM مستمرة.
    let tries=0;
