...
const CASE_BROKEN_FAUCET = {
  ...
  floorPlanPuzzle: {
    ...
    resultText: 'الغرفة البعيدة فيها مستندات ميراث تخص يوسف، وده بيدل إن عادل لقى المستندات دي بالصدفة وهو بيدور على مصدر التسريب.',
...
(() => {
  ...
  c.floorPlanPuzzle.resultText = 'المسار بيوصل من المطبخ للغرفة المقفولة اللي يوسف كان بيروحها عشان مستندات الميراث، وده بيربط مكان الجريمة مباشرة بتحركاته داخل الفيلا.';
  c.floorPlanPuzzle.resultEvidenceIds = ['youssef_b_route'];
  c.conclusiveEvidenceIds = ['youssef_b_inheritance','hidden_room_documents','youssef_b_route'];
  ...
  if (y && !y.questions.some(q => q.closesInterrogation)) y.questions.push({
    ...
    a:'(بيخفض صوته) "كنت هناك فعلًا. عادل وصل للأوضة بالصدفة وشاف الورق. خفت الموضوع يوصل لإخواتي قبل ما أحسم النزاع، حاولت أمنعه والمواجهة خرجت عن السيطرة."'
  });
