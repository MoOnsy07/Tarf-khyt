...
(() => {
  ...
  /* ----------------------------------------------------------
     العيادة السرية — تفسير رسالة غادة المريبة بعد ظهور سجل
     المستشفى، من غير إضافة دليل جديد يغيّر عتبة فتح الاتهام.
     ---------------------------------------------------------- */
  if(typeof CASE_SECRET_CLINIC !== 'undefined'){
    const c = CASE_SECRET_CLINIC;
    const ghada = (c.suspects || []).find(s => s && s.id === 'assistant_ghada');
    if(ghada){
      ghada.questions = ghada.questions || [];
      const exists = ghada.questions.some(q => String(q.q || '').includes('إيمان سافرت'));
      if(!exists){
    ...
    const suspicious = getEvidence(c,'donia_suspicious_message');
    if(suspicious){
  ...
  try{
    window.__TARAF_DISCOVERY_PREACCUSATION_AUDIT__ = {
      ...
      reviewed:['room-307','postponed-engagement','coded-message','false-rumor','lost-wallet','wedding-gold','secret-clinic'],
      changed:['postponed-engagement','coded-message','lost-wallet','wedding-gold','secret-clinic']
    };
  }catch(_){}
})();
